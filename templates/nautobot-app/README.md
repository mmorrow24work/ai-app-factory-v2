...
Scaffold for a Nautobot App project driven by the ai-app-factory-v2 unattended (Lane B)
...
`PYTHON_PACKAGE` (defaults to a slugified repo name), `AUTHOR_NAME`, `NAUTOBOT_VERSION`
(defaults to `^3.0.0`), `ADDITIONAL_CONVENTIONS`, `FACTORY_REPO`.

The shipped `app/` package directory is a literal, un-templated name -- see `CLAUDE.md.tmpl`'s
"Repo map" for why, and rename it to match `{{PYTHON_PACKAGE}}` if it differs from `app`.
